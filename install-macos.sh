...
# CS3223 Assignment 1
# Script to install PostgreSQL 18.1 on MacOS
...
# This package is needed to build Postgres 18.1.
# The following lines should work on both Mac Intel and ARM.
# If you still get an error, you may need to change the icu4c path slightly,
# see: https://viggy28.dev/article/postgres-v16-icu-installation-issue/

# Find the latest icu4c version installed. Install if not found
icu_formula=$(brew list --formula | grep '^icu4c@' | sort -V | tail -1)

if [ -z "$icu_formula" ]; then
  icu_formula=$(brew search --formula icu4c@ | sort -V | tail -1)
  brew install "$icu_formula"
fi

export PKG_CONFIG_PATH=$(brew --prefix $icu_formula)/lib/pkgconfig/:${PKG_CONFIG_PATH:-""}
